...
if [ $? -ne 0 ]; then
    ...
    echo ""
    echo "Examples:"
    echo "  $0 5m    # Schedule to turn OFF in 5 minutes"
    echo "  $0 2h    # Schedule to turn OFF in 2 hours"
    echo "  $0 3d    # Schedule to turn OFF in 3 days"
    echo "  $0 30    # Schedule to turn OFF in 30 minutes (no unit = minutes)"
    exit 1
fi

NOW=$(date +%s)
TURN_OFF_TIME=$((($NOW + ($MINUTES_FROM_NOW * 60)) * 1000))   # Convert to milliseconds
...
echo "🔴 Scheduling flag to turn OFF..."
echo "📅 Flag: ${FLAG_KEY}"
echo "📅 Project: ${PROJECT_KEY}"
echo "📅 Environment: ${ENVIRONMENT}"
echo "⏱️  Time from now: ${TIME_DISPLAY}"
echo "📅 Scheduled time: $(date -r $(($TURN_OFF_TIME/1000)) '+%Y-%m-%d %H:%M:%S')"
echo ""

# Schedule flag to turn OFF
...
RESPONSE=$(curl -s -w "\n%{http_code}" -X POST \
  ...
  -d "{
    \"comment\": \"Scheduled to turn OFF flag in ${MINUTES_FROM_NOW} minutes\",
    \"executionDate\": ${TURN_OFF_TIME},
    \"instructions\": [
      {
        \"kind\": \"turnFlagOff\"
      }
